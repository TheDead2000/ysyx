...
void SimAxi4::mmio_device_init() {
    ...
    assert(mmio.add_dev(MMIO_BASE,0x40000,mydevices));
...
void SimAxi4::update_input() {
    axi4_ref <32, 32, 4> mmio_sigs_ref(mmio_sigs); // 用于 soc-simulator 的内部信号处理
    axi4_ref <32, 32, 4> mmio_ref(mmio_ptr); // 指向 npc 与 soc-simulator 的交互信号
