...
void Devicevga::write(paddr_t addr, word_t data, uint32_t len)
{
    ...
    uint32_t offset = addr - deviceinfo.at(1).addr;
    uint32_t *dest = vgaregs.fbbuff + (offset / sizeof(uint32_t)); // 自动按uint32_t大小计算
        ...
        *dest = (uint32_t)data;
        ...
        case 8:
            ...
            else
            {
                ...
                *(uint32_t *)(dest + 1) = (uint32_t)(data >> 32);
            }
            break;
        default:
            printf("unsupported len:%d\n", len);
            assert(0);
            break;
        }
    }
