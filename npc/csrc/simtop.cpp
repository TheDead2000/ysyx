...
#endif
    delete top;       // 先销毁 top
    delete contextp;  // 再销毁 contextp
    // delete mem;    // 如果有 mem，确保它在最后
    cout << "SimtopEnd!" << endl;
}
 ...
 * @return uint32_t
 */
uint32_t Simtop::getRegVal(int idx) {
 ...
 * @return uint32_t
 */
uint32_t Simtop::getRegVal(const char* str) {
    uint32_t val;
    if (!strcmp(str, "pc")) {
        val = this->pc;
        return val;
...
void Simtop::setPC(uint32_t val) {
    this->pc = val;
}
...
void Simtop::GPRregsReset(void) {
    for(int i = 0; i < 32 ; i++){
        this->registerfile[i] = 0;
    }
}
void Simtop::addCommitedInst(uint32_t inst_pc, uint32_t inst_data) {
